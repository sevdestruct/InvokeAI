...
class InvokeAIAppConfig(BaseSettings):
    ...
    deepcache_interval:             int = Field(default=1, ge=1,             description="DeepCache step interval for SD 1.x / SDXL. 1 disables it. Values >1 reuse the deep UNet block outputs for that many steps, recomputing only every Nth step -- measured ~1.8x (interval=2) to ~2.9x (interval=5) faster on Apple Silicon, with a small quality change that grows with the interval. Requires the `DeepCache` package. Automatically disabled with ControlNet/T2I-Adapter or sequential guidance.")
    flux_first_block_cache_threshold: float = Field(default=0.0, ge=0.0, le=1.0, description="FLUX FirstBlockCache residual-reuse threshold. 0 disables it. When >0, the denoiser skips the remaining transformer blocks on steps where the first block's output changes by less than this relative-L1 threshold, reusing the previous step's cached residual -- measured ~1.5-2x faster FLUX denoising, with a quality change that grows with the threshold (typical range 0.05-0.15). Automatically disabled with ControlNet or IP-Adapter.")
    flux_first_block_cache_error_budget: float = Field(default=0.0, ge=0.0, description="Optional error-bounded mode for FLUX FirstBlockCache (TeaCache/MagCache family). 0 uses the plain instantaneous threshold above. When >0, the denoiser instead accumulates the first block's relative-L1 change across steps and forces a full recompute once the accumulation exceeds this budget, which bounds the total drift from consecutive skips (typically steadier quality than a fixed threshold at similar speed; tune ~0.1-0.4). Takes precedence over the threshold when set.")
    flux_flash_attention:          bool = Field(default=False,              description="Experimental: route the UNMASKED FLUX attention sites through a fused Metal flash-attention kernel when a vetted one is installed. Off by default and a no-op unless a compatible MPS kernel is present and passes a numerical parity check; masked sites (regional prompting / IP-Adapter) always use stock SDPA. Use scripts/flash_attn_probe.py to vet a kernel before enabling.")
